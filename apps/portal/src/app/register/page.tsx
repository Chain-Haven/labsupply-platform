...
export default function RegisterPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch(`/api/v1/auth/accept-invite?token=${inviteToken}`);
                if (res.ok) {
                ...
                // Non-critical: user can still register normally
            }
        })();
    }, [inviteToken]);
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            if (result.success) {
                // If this is an invite registration, accept the invite then go to dashboard
                if (inviteToken) {
                    try {
                        await fetch(`/api/v1/auth/accept-invite?token=${inviteToken}`, {
                            method: 'POST',
                            credentials: 'include',
                        });
                    } catch {
                        // Best-effort: they can re-accept from the link later
                    }
                    const dest = inviteInfo?.scope === 'admin' ? '/admin' : '/dashboard';
                    router.push(dest);
                } else {
                ...
                <Card className="border-white/10 bg-white/5 backdrop-blur-xl">
                    <CardHeader className="text-center">
                        ...
                        <CardDescription className="text-white/60">
                            {isInvite
                                ? `You've been invited to join ${inviteInfo.merchant_name || 'a team'}`
                            ...
                            <div className="mb-4 p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
                                <div className="flex items-start gap-2">
                                    ...
                                    <p className="text-xs text-indigo-300">
                                        You&apos;re joining as <strong>{inviteInfo.role.replace('MERCHANT_', '').replace(/_/g, ' ')}</strong>.
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        readOnly={isInvite}
                                        className={`pl-10 bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-violet-500 ${isInvite ? 'opacity-60 cursor-not-allowed' : ''}`}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="p-3 rounded-lg bg-violet-500/10 border border-violet-500/20">
                                <div className="flex items-start gap-2">
                                    ...
                                    <p className="text-xs text-violet-300">
                                        {isInvite
                                            ? 'After verifying your email, you will be added to the team and directed to the dashboard.'
                                            : 'After verifying your email, you will complete a short onboarding process with your business details and compliance documents.'}
                                    </p>
                                </div>
                            </div>
